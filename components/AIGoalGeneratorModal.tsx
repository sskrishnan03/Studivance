...
const AIGoalGeneratorModal: React.FC<AIGoalGeneratorModalProps> = ({ isOpen, onClose, onSelectGoal }) => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            const result = await groqChatJSON<{ goals: GeneratedGoal[] }>([
                groqSystemMsg(`You are a SMART goal creator. Based on the given objective, generate 3 distinct SMART (Specific, Measurable, Achievable, Relevant, Time-bound) goal suggestions for a student. For targetDate, suggest a realistic date from today in YYYY-MM-DD format. Return valid JSON with a "goals" array where each item has: title (string), description (string), targetDate (string YYYY-MM-DD).`),
                groqUserMsg(`Generate SMART goals for: ${prompt}`)
            ]);
