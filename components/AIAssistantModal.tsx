...
import { groqChat, groqChatJSON, groqSystemMsg, groqUserMsg } from '../lib/groq';
        ...
        try {
            if (action === 'quiz') {
                ...
                const prompts: Record<'summarize' | 'explain', string> = {
                    summarize: `Summarize the following note content from "${subjectTitle}". Provide key bullet points only. Format in HTML using <ul> and <li> tags.\n\n${plainTextContent}`,
                    explain: `Explain the core concepts from the following note content from "${subjectTitle}" in simple, easy-to-understand language as if teaching a beginner. Format in HTML with clear paragraphs.\n\n${plainTextContent}`,
                };
